...
def aling_table(array: list):
    global width
    size = len(array)
    all_digits = 0

    for each in array:
        all_digits += len(str(each))
    table_width = all_digits + size + 1

    return (width - table_width) // 2


def array_table(array: list, line: int):
    size = len(array)
    center_x = aling_table(array)

    gotoxy(center_x, line)
    stdout.write("┌")
    for i in range(size):
        stdout.write("─" * len(str(array[i])))
        if (i + 1 < size):
            stdout.write("┬")
    stdout.write("┐")

    gotoxy(center_x, line + 1)
    stdout.write("│")
    for element in array:
        stdout.write(str(element))
        stdout.write("│")

    gotoxy(center_x, line + 2)
    stdout.write("└")
    for i in range(size):
        stdout.write("─" * len(str(array[i])))
        if (i + 1 < size):
